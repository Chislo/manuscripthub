...
def generate_readiness_pdf(result, journal_name):
    ...
    pdf.set_font("Helvetica", size=10)
    pdf.cell(0, 10, clean_text_for_pdf(f"Generated by ManuscriptHub on {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}"), ln=True, align="C")
    pdf.ln(10)
    ...
    pdf.multi_cell(pdf.epw, 8, clean_text_for_pdf(f"Verdict: {result.get('overall_verdict', 'N/A')}"))
    pdf.ln(5)
    
    # Details
    pdf.set_font("Helvetica", style="B", size=12)
    pdf.cell(0, 8, clean_text_for_pdf(f"Target Journal: {journal_name}"), ln=True)
    pdf.ln(5)
    ...
    for section, title in [("abstract_feedback", "Abstract Analysis"), 
        ...
        pdf.set_font("Helvetica", style="B", size=12)
        pdf.cell(0, 10, clean_text_for_pdf(f"{title} (Score: {fb.get('score', 'N/A')}/100)"), ln=True)
        pdf.set_font("Helvetica", size=10)
        ...
        if fb.get("issues"):
            ...
            for issue in fb["issues"]:
                pdf.multi_cell(pdf.epw, 6, clean_text_for_pdf(f"- {issue}"))
        
        if fb.get("suggestion"):
            pdf.set_font("Helvetica", style="B", size=10)
            pdf.multi_cell(pdf.epw, 6, clean_text_for_pdf(f"Recommendation: {fb['suggestion']}"))
        pdf.ln(5)
    ...
    for i, item in enumerate(result.get("action_items", []), 1):
        pdf.multi_cell(pdf.epw, 6, clean_text_for_pdf(f"{i}. {item}"))
    pdf.ln(10)
    ...
    for chk in result.get("compliance_checklist", []):
        ...
        pdf.multi_cell(pdf.epw, 6, clean_text_for_pdf(f"[{status}] {chk.get('item')}: {chk.get('note')}"))
    pdf.ln(5)
